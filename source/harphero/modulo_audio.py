...
def pitch_sample(sound, semitones):
    ...
    new_positions = np.linspace(0,original_length - 1,new_length)
    if original.ndim == 2:
        left = np.interp(new_positions,old_positions,original[:, 0])
        right = np.interp(new_positions,old_positions,original[:, 1])
